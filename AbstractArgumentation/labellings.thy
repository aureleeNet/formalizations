...
declare conflictfreeLab_def[Defs]
...
declare admissibleLab_def[Defs]
...
declare completeLab2_def[Defs]

lemma complete_defEq: \<open>completeLab\<^sup>\<A> att Lab \<longleftrightarrow> completeLab2\<^sup>\<A> att Lab\<close>
  by (smt (verit, ccfv_SIG) Label.exhaust Label.simps(3) Label.simps(5) admissibleLab_def completeLab2_def completeLab_def inset_def legallyIn_def legallyOut_def legallyUndec_def outset_def undecset_def)
...
definition groundedLab :: \<open>'a Set \<Rightarrow> 'a Rel \<Rightarrow> 'a Labelling \<Rightarrow> bool\<close> ("groundedLab\<^sup>_")
  (* where \<open>grounded\<^sup>\<A> att Lab \<equiv> minimal (complete\<^sup>\<A> att) Lab in\<close> *)
...
definition groundedLab2 :: \<open>'a Set \<Rightarrow> 'a Rel \<Rightarrow> 'a Labelling \<Rightarrow> bool\<close> ("groundedLab2\<^sup>_")
  (* where \<open>grounded2\<^sup>\<A> att Lab \<equiv> least (complete\<^sup>\<A> att) Lab in\<close> *)
  where \<open>groundedLab2\<^sup>\<A> att Lab \<equiv> least\<^sup>\<A> (completeLab\<^sup>\<A> att) Lab in\<close>
declare groundedLab_def[Defs] groundedLab2_def[Defs] 
...
declare semistableLab_def[Defs]
